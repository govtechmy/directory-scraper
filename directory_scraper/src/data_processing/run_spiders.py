...
def filter_custom_logs(LOG_FILE_PATH=LOG_FILE_PATH):
    ...
    excluding Scrapy's internal logs.
    ...
    The name of the main log file to filter (defaults to the global LOG_FILE_NAME variable).
    """
    ...
    try:
        with open(input_log, "r") as infile:
            with open(output_log, "w") as outfile:
                for line in infile:
                    # Check if the line contains the custom logger's identifier (__name__)
                    if custom_logger_name in line:
                        outfile.write(line)
