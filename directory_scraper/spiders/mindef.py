...
class MINDEFSpider(scrapy.Spider):
    name = "mindef"
    ...
    start_urls = [
        ...
        "https://direktori.mod.gov.my/index.php/mindef/category/bahagian-akaun/13",
    ...
    # start_urls = ["https://direktori.mod.gov.my/index.php/mindef/category/bahagian-khidmat-pengurusan/2"]
            ...
            yield scrapy.Request(
                ...
                meta={
                    ...
                    "playwright_page_methods": [
                        PageMethod("wait_for_selector", "joomla-hidden-mail"),
                    ],
                }
            )
    ...
    async def parse_item(self, response):
        ...
        await page.wait_for_selector('joomla-hidden-mail')
        
        print(f"Processing URL: {response.url}")

        current_division = response.css("h1 ::text").get()
        division_sort_order = min([idx for idx, base_url in enumerate(self.directory_lst) if base_url in response.url])+1
        ...
        for sort_order, data_card in enumerate(response.css("div[class='uk-overflow-hidden']")):
            data = {
                ...
                "unit": data_card.css("a::text").getall(),
                "page_number": page_number[0] if (page_number := re.findall("\d+$", response.url)) else 1
            }
            yield data
